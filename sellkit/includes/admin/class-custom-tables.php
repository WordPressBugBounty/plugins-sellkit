...
class Sellkit_Custom_Tables {
	...
	public function get_generated_coupons() {
		...
		foreach ( $query->posts as $post ) {
			...
			$used_by        = get_post_meta( $post->ID, '_used_by', true );

			if ( empty( $customer_email ) && ! empty( $used_by ) ) {
				$used_by_customer = is_email( $used_by ) ? $used_by : '';

				if ( empty( $used_by_customer ) ) {
					$used_by_customer = get_userdata( intval( $used_by ) )->user_email;
				}

				$customer_email = [ $used_by_customer ];
			}
