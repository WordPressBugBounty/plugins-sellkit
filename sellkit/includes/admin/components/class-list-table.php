...
class Sellkit_Admin_List_Table {
	...
	private function handle_post_export_all_funnel() {
		...
		$global_checkout_id = intval( get_option( Checkout::SELLKIT_GLOBAL_CHECKOUT_OPTION, 0 ) );

		foreach ( $posts as $post ) {
			if ( $post->ID === $global_checkout_id ) {
				continue;
			}
	...
	private function handle_post_import_funnel() {
		...
		if ( 'multi' === $data->type && 'checkout' === $page ) {
			wp_send_json_error( esc_html__( 'You can not import multiple funnels to the global checkout.', 'sellkit' ) );
		}
