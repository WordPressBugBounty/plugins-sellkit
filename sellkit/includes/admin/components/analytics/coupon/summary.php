...
class Summary {
	...
	public function set_discounts_details() {
		...
		$this->data['revenue']        = ! empty( $monetary_data[0]['revenue'] ) ?  round( $monetary_data[0]['revenue'], 2 ) : 0;
		$this->data['total_discount'] = ! empty( $monetary_data[0]['total_discount'] ) ?  round( $monetary_data[0]['total_discount'], 2 ) : 0;
		...
		if ( ! empty( $this->data['used_coupon'] ) ) {
			$this->data['conversion_rate'] = ( $this->data['customers'] / $this->data['created_coupon'] ) * 100;
