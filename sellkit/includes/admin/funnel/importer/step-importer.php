...
class Step_Importer {
	...
	public function import_to_funnel( $data, $funnel_id, $origin_node = null, $target_node = null, $target_index = 0 ) {
		...
		if ( 'last-node' !== $origin_node && ! empty( $origin_node ) && isset( $new_funnel_step_data[ $origin_node ] ) && is_object( $new_funnel_step_data[ $origin_node ]['type'] ) ) {
		...
		if (
			'last-node' !== $origin_node &&
			isset( $new_funnel_step_data[ $new_step_id ] ) &&
			isset( $new_funnel_step_data[ $new_step_id ]['origin_node'] ) &&
			'last-node' === $new_funnel_step_data[ $new_step_id ]['origin_node'] &&
			(
				'decision' !== $new_funnel_step_data[ $new_step_id ]['type']['key'] ||
				'upsell' !== $new_funnel_step_data[ $new_step_id ]['type']['key'] ||
				'downsell' !== $new_funnel_step_data[ $new_step_id ]['type']['key']
			)
		) {
			unset( $new_funnel_step_data[ $new_step_id ]['origin_node'] );
		}
