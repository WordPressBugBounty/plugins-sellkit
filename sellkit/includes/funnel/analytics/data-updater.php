...
class Data_Updater {
	...
	/**
	 * Adds new order log for accepted upsell.
	 *
	 * @since 1.9.2
	 * @param double|int $total Upsell total price.
	 */
	public function add_new_upsell_revenue_log( $total ) {
		$data = self::has_funnel_data();
		...
		//phpcs:disable
		$wpdb->query(
			$wpdb->prepare( "UPDATE `$table` SET upsell_revenue = %s WHERE funnel_id = %d AND applied_at > %s", $data[ 'upsell_revenue' ] + $total, $this->funnel_id, $this->today_min_time )
		);
		//phpcs:enable
	}
}
